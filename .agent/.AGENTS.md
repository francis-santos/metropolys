...
## Automação de Qualidade (E2E OBRIGATÓRIO)
- **REGRA ABSOLUTA DE QA:** Sempre que você modificar qualquer arquivo dentro do diretório apps/web (Frontend) ou realizar manutenções visuais no repositório, você é OBRIGADO a rodar npm run test:playwright silenciosamente no terminal e validar se os testes continuam passando ANTES de dar sua resposta final ao usuário. A qualidade visual do Metropolys é inegociável.
