...
export class AiService implements OnModuleInit {
  ...
  // Helper: call Gemini with rate limit recovery and timeout fallback
  private async promptGemini(prompt: string, fallbackJson: any): Promise<any> {
    if (!this.hasKey || !this.genAI) return fallbackJson;
  ...
  async getBotDecision(
    ...
    // A. Local Rule-Based Fallback logic
    let fallbackDecision: BotDecision = { action: 'PASS', rationale: 'Pensando estrategicamente.' };
    ...
    if (actionType === 'BUY') {
      ...
      // Accept if net value makes sense
      if (offerCash >= 150) {
    ...
    if (!this.hasKey) return fallbackDecision;
...
Você DEVE responder estritamente no formato JSON:
{
  ...
  // 2. AI Game Master Economic Events (Triggered every 5 rounds)
  async generateCityEvent(round: number, standings: any[]): Promise<CityEvent> {
    const list = [
    ...
    if (!this.hasKey) return fallbackEvent;
...
Você DEVE responder estritamente no formato JSON:
{
  ...
  // 3. AI Narrator Commentaries (High impact events)
  async generateNewsCommentary(eventDescription: string): Promise<NewsHeadline> {
    const fallbackNews: NewsHeadline = {
    ...
    if (!this.hasKey) return fallbackNews;
