...
export class RoomsController {
  ...
  @Post()
  createRoom(@Body() body: { cityCode: string; hostName: string; hostColor: string; hostPin?: string }) {
    if (!body.hostName) {
  ...
  @Post(':code/start')
  startMatch(@Param('code') code: string) {
    return this.roomsService.startMatch(code);
  }

  @Post(':code/roll')
  rollDice(@Param('code') code: string, @Body() body: { playerId: string }) {
    if (!body.playerId) {
  ...
  @Post(':code/end-turn')
  endTurn(@Param('code') code: string, @Body() body: { playerId: string }) {
    if (!body.playerId) {
      throw new BadRequestException('Player ID is required');
    }
  ...
  @Post(':code/auctions')
  startAuction(@Param('code') code: string, @Body() body: { slotId: number }) {
  ...
  proposeTrade(
    ...
    return this.roomsService.proposeTrade(
      code,
      body.senderId,
      body.receiverId,
      body.offerCash,
